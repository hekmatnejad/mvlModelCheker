...
mv_interval::mv_interval(lattice_node* intervals, int size) throw() :
intervals_(intervals), int_size_(size) 
{
    map_intervals_ = new std::map<string,mv_interval*>();
    map_all_intervals_ = new std::map<std::pair<float,float>,mv_interval*>();
...
mv_interval::mv_interval(float low, float high): mv_interval(){
    //mv_interval();
...
mv_interval* mv_interval::join_mv(mv_interval* left, mv_interval* right){
    ...
    if(map_all_intervals_->find(std::make_pair(low,high))==map_all_intervals_->end()){
        mv_interval* res = new mv_interval(low, high);
...
mv_interval* mv_interval::psi_mv(mv_interval* base, mv_interval* given){
    ...
    if(map_all_intervals_->find(std::make_pair(new_low,new_high))==map_all_intervals_->end()){
        mv_interval* res = new mv_interval(new_low, new_high);
...
mv_interval* create_interval_set(string prefix, int num_nodes)
{
    ...
    mv_interval* itv = new mv_interval(nodes, num_nodes);
...
//OR    
    mv_interval intv = *mvspot::create_interval_set("l",5);
    ...
    mv_interval it1= mv_interval(0.125,1);
    cout << "it1: " << *it1.getTo_lattice_() << endl<<endl;   
    mv_interval it2= mv_interval(0.625,1);
